<style>
    /* ── Sidebar Utama (Tema: Obsidian Black & Royal Blue) ── */
    .sidebar {
    ...
    /* Menu Aktif dengan Warna Royal Blue Gelap */
    .nav-row.active { background: #172554; color: #eff6ff; } 
    .nav-row.active .nr-icon-wrap { background: #1e3a8a; border-color: #1d4ed8; }
    .nav-row.active .nr-icon-wrap i { color: #60a5fa; }
    ...
    .nav-row:hover .nr-icon-wrap i { color: #3b82f6; }
    ...
    .sub-row:hover { background: #14171c; color: #3b82f6; }
    .sub-row.active { color: #3b82f6; background: #14171c; font-weight: 500; }
    .sub-dot { width: 4px; height: 4px; border-radius: 50%; background: #2b303b; flex-shrink: 0; transition: background .15s ease; }
    .sub-row:hover .sub-dot, .sub-row.active .sub-dot { background: #3b82f6; box-shadow: 0 0 5px rgba(59, 130, 246, 0.5); } /* Efek glow biru */
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sb-nav">
        ...
        <div class="parent {{ request()->routeIs('admin.kasir.*') ? 'open' : '' }}" id="p-kasir">
            ...
            <div class="sub">
                <a href="{{ route('admin.kasir.pos') }}" class="sub-row {{ request()->routeIs('admin.kasir.pos') ? 'active' : '' }}"><span class="sub-dot"></span>Transaksi POS</a>
            </div>
        </div>
    </nav>
